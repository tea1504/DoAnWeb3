...
@endsection
@section('content')
<div class="container-fluid" ng-controller="quanlythemmoiController" ng-init="dv_ten = '{{old('dv_ten')}}'; dvql_ma = '{{old('dvql_ma')}}'">
    ...
    <div class="row">
        <div class="col-md-12">
            <div class="card">
                <div class="card-header bg-cyan h1 font-weight-bold">Thêm mới đơn vị</div>
                <div class="card-body">
                    <form name="frmCreate" id="frmCreate" method="POST" action="{{route('admin.donvi.store')}}" enctype="multipart/form-data" novalidate>
                        {{ csrf_field() }}
                        <div class="form-group">
                            <label for="dv_ten">Tên đơn vị :</label>
                            <input type="text" name="dv_ten" id="dv_ten" ng-model="dv_ten" ng-class="['form-control', frmCreate.dv_ten.$touched?frmCreate.dv_ten.$invalid?'is-invalid':'is-valid':'']" ng-model="dv_ten" ng-required="true" ng-minlength="3" ng-maxlength="50">
                            <div class="invalid-feedback">
                                <span ng-if="frmCreate.dv_ten.$error.required">Bạn phải nhập vào trường này</span>
                                <span ng-if="frmCreate.dv_ten.$error.minlength">Bạn nhập quá ngắn</span>
                                <span ng-if="frmCreate.dv_ten.$error.maxlength">Bạn nhập quá dài</span>
                            </div>
                        </div>
                        <div class="form-group">
                            <label for="dvql_ma">Đơn vị quản lý :</label>
                            <select name="dvql_ma" id="dvql_ma" ng-model="dvql_ma" ng-class="['form-control', frmCreate.dvql_ma.$touched?frmCreate.dvql_ma.$invalid?'is-invalid':'is-valid':'']" ng-required="true">
                                <option value=""></option>
