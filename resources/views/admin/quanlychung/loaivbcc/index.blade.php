...
@section('title')
Danh sách loại văn bằng chứng chỉ
@endsection
@section('custom-css')
...
<ol class="breadcrumb float-sm-right">
    ...
    <li class="breadcrumb-item active">Danh sách loại văn bằng chứng chỉ</li>
</ol>
@endsection
@section('content')
    ...
    <div class="row">
        <div class="col text-right">
            <div class="btn-group" role="group">
                <a href="{{route('admin.loaivbcc.create')}}" id="add" class="btn btn-dark"><i class="fas fa-plus-circle"></i> Thêm mới</a>
            </div>
        </div>
    </div>
    <div class="row">
                ...
                <div class="card-body">
                    <table class="table table-hover table-responsive table-striped table-bordered table-head-fixed w-100 order-column" id="myTable">
                        <thead>
                            <tr>
                                <th>#</th>
                                <th>Tên loại văn bằng chứng chỉ</th>
                                <th>Ngày tạo mới</th>
                                <th>Ngày cập nhật</th>
                                <th>Actions</th>
                            </tr>
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td class="align-middle">{{$lvbcc->lvbcc_ten}}</td>
                                <td class="align-middle text-center">{{$lvbcc->lvbcc_taoMoi->format('d/m/Y H:m:s')}}</td>
                                <td class="align-middle text-center">{{$lvbcc->lvbcc_capNhat->format('d/m/Y H:m:s')}}</td>
                                <td class="align-middle text-center">
                                <a href="{{route('admin.loaivbcc.edit', ['id' => $lvbcc->lvbcc_ma])}}" class="btn btn-success btn-sm" class="btn btn-success" data-toggle="tooltip" data-placement="top" title="Sửa"><i class="fa fa-edit" aria-hidden="true"></i></a>
                                    <form class="fDelete btn p-0" method="POST" action="{{route('admin.loaivbcc.destroy', ['id'=>$lvbcc->lvbcc_ma])}}">
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        Swal.fire({
                            ...
                            window.location = "{{route('admin.loaivbcc.index')}}"
                        })
                    },
