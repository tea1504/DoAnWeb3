...
<ol class="breadcrumb float-sm-right">
    ...
    <li class="breadcrumb-item active">Danh sách huyện</li>
</ol>
@endsection
@section('content')
    ...
    <div class="row">
        <div class="col text-right">
            <div class="btn-group" role="group">
                <a href="{{route('admin.huyen.create')}}" id="add" class="btn btn-dark"><i class="fas fa-plus-circle"></i> Thêm mới</a>
            </div>
        </div>
    </div>
    <div class="row">
                    ...
                    <table class="table table-hover table-responsive table-striped table-bordered table-head-fixed w-100 order-column" id="myTable">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td class="align-middle">{{$h->h_ten}}</td>
                                <td class="align-middle">{{$h->tinh->t_ten}}</td>
                                <td class="align-middle text-center">{{$h->h_taoMoi->format('d/m/Y H:m:s')}}</td>
                                <td class="align-middle text-center">{{$h->h_capNhat->format('d/m/Y H:m:s')}}</td>
                                <td class="align-middle text-center">
                                <a href="{{route('admin.huyen.edit', ['id' => $h->h_ma])}}" class="btn btn-success btn-sm" class="btn btn-success" data-toggle="tooltip" data-placement="top" title="Sửa"><i class="fa fa-edit" aria-hidden="true"></i></a>
                                    <form class="fDelete btn p-0" method="POST" action="{{route('admin.huyen.destroy', ['id'=>$h->h_ma])}}">
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        Swal.fire({
                            ...
                            window.location = "{{route('admin.huyen.index')}}"
                        })
                    },
