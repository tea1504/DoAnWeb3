...
<ol class="breadcrumb float-sm-right">
    ...
    <li class="breadcrumb-item active">Cập nhật huyện</li>
</ol>
@endsection
@section('content')
<div class="container-fluid" ng-controller="quanlythemmoiController" ng-init="h_ten = '{{old('h_ten', $h->h_ten)}}'; t_ma = '{{old('t_ma', $h->t_ma)}}'">
    ...
    <div class="row">
        <div class="col-md-12">
            <div class="card">
                <div class="card-header bg-cyan h1 font-weight-bold">Cập nhật huyện</div>
                <div class="card-body">
                    <form name="frmEdit" id="frmEdit" method="POST" action="{{route('admin.huyen.update', ['id' => $h->h_ma])}}" enctype="multipart/form-data" novalidate>
                        {{ csrf_field() }}
                        <input type="hidden" name="_method" value="PUT">
                        <div class="form-group">
                            <label for="h_ten">Tên huyện :</label>
                            <input type="text" name="h_ten" id="h_ten" ng-model="h_ten" ng-class="['form-control', frmEdit.h_ten.$invalid?'is-invalid':'is-valid']" ng-model="h_ten" ng-required="true" ng-minlength="3" ng-maxlength="50">
                            <div class="invalid-feedback">
                                <span ng-if="frmEdit.h_ten.$error.required">Bạn phải nhập vào trường này</span>
                                <span ng-if="frmEdit.h_ten.$error.minlength">Bạn nhập quá ngắn</span>
                                <span ng-if="frmEdit.h_ten.$error.maxlength">Bạn nhập quá dài</span>
                            </div>
                        </div>
                        <div class="form-group">
                            <label for="t_ma">Đơn vị quản lý :</label>
                            <select name="t_ma" id="t_ma" ng-model="t_ma" ng-class="['form-control', frmEdit.t_ma.$invalid?'is-invalid':'is-valid']" ng-required="true">
                                <option value=""></option>
                                @foreach($dst as $t)
