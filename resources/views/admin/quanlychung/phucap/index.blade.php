    ...
    <div class="row">
        <div class="col text-right">
            <div class="btn-group" role="group">
                <a href="{{route('admin.phucap.create')}}" id="add" class="btn btn-dark"><i class="fas fa-plus-circle"></i> Thêm mới</a>
            </div>
        </div>
    </div>
    <div class="row">
                    ...
                    <table class="table table-hover table-responsive table-striped table-bordered table-head-fixed w-100 order-column" id="myTable">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td class="align-middle">{{$pc->pc_ten}}</td>
                                <td class="align-middle text-center">{{$pc->pc_heSoPhuCap}}</td>
                                <td class="align-middle text-center">{{$pc->pc_taoMoi->format('d/m/Y H:m:s')}}</td>
                                <td class="align-middle text-center">{{$pc->pc_capNhat->format('d/m/Y H:m:s')}}</td>
                                <td class="align-middle text-center">
                                <a href="{{route('admin.phucap.edit', ['id' => $pc->pc_ma])}}" class="btn btn-success btn-sm" class="btn btn-success" data-toggle="tooltip" data-placement="top" title="Sửa"><i class="fa fa-edit" aria-hidden="true"></i></a>
                                    <form class="fDelete btn p-0" method="POST" action="{{route('admin.phucap.destroy', ['id'=>$pc->pc_ma])}}">
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        Swal.fire({
                            ...
                            window.location = "{{route('admin.phucap.index')}}"
                        })
                    },
