...
<ol class="breadcrumb float-sm-right">
    ...
    <li class="breadcrumb-item active">Danh sách dân tộc</li>
</ol>
@endsection
@section('content')
    ...
    <div class="row">
        <div class="col text-right">
            <div class="btn-group" role="group">
                <a href="{{route('admin.dantoc.create')}}" id="add" class="btn btn-dark"><i class="fas fa-plus-circle"></i> Thêm mới</a>
            </div>
        </div>
    </div>
    <div class="row">
                    ...
                    <table class="table table-hover table-responsive table-striped table-bordered table-head-fixed w-100 order-column" id="myTable">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td class="align-middle">{{$dt->dt_ten}}</td>
                                <td class="align-middle text-center">{{$dt->dt_taoMoi->format('d/m/Y H:m:s')}}</td>
                                <td class="align-middle text-center">{{$dt->dt_capNhat->format('d/m/Y H:m:s')}}</td>
                                <td class="align-middle text-center">
                                <a href="{{route('admin.dantoc.edit', ['id' => $dt->dt_ma])}}" class="btn btn-success btn-sm" class="btn btn-success" data-toggle="tooltip" data-placement="top" title="Sửa"><i class="fa fa-edit" aria-hidden="true"></i></a>
                                    <form class="fDelete btn p-0" method="POST" action="{{route('admin.dantoc.destroy', ['id'=>$dt->dt_ma])}}">
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        Swal.fire({
                            ...
                            window.location = "{{route('admin.dantoc.index')}}"
                        })
                    },
