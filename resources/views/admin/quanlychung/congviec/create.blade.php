...
@endsection
@section('content')
<div class="container-fluid" ng-controller="quanlythemmoiController" ng-init="cv_ten = '{{old('cv_ten')}}'">
    ...
    <div class="row">
        <div class="col-md-12">
            <div class="card">
                <div class="card-header bg-cyan h1 font-weight-bold">Thêm mới công việc</div>
                <div class="card-body">
                    <form name="frmCreate" id="frmCreate" method="POST" action="{{route('admin.congviec.store')}}" enctype="multipart/form-data" novalidate>
                        {{ csrf_field() }}
                        <div class="form-group">
                            <label for="cv_ten">Tên công việc :</label>
                            <input type="text" name="cv_ten" id="cv_ten" ng-model="cv_ten" ng-class="['form-control', frmCreate.cv_ten.$touched?frmCreate.cv_ten.$invalid?'is-invalid':'is-valid':'']" ng-model="cv_ten" ng-required="true" ng-minlength="3" ng-maxlength="50">
                            <div class="invalid-feedback">
                                <span ng-if="frmCreate.cv_ten.$error.required">Bạn phải nhập vào trường này</span>
                                <span ng-if="frmCreate.cv_ten.$error.minlength">Bạn nhập quá ngắn</span>
                                <span ng-if="frmCreate.cv_ten.$error.maxlength">Bạn nhập quá dài</span>
                            </div>
                        </div>
                        <div class="form-group">
                            <label for="cv_moTa">Mô tả :</label>
                            <textarea name="cv_moTa" id="cv_moTa" cols="30" rows="10" class="textarea">{{old('cv_moTa')}}</textarea>
                        </div>
                        <div class="form-group">
